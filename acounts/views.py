...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f'Welcome "{user}" Your Acount Was Created ! Now Login Please ')
...
def home(request):
    ...
    #   ordersall |= Order.objects.filter(
    #      id=order.id).order_by('-date_created')

    #orders = ordersall[:6]
    ######################
    # get all orders for all customers for the user
    ordersall = Order.objects.filter(
        customer__user__username=f'{user.username}').order_by('-date_created')
    # get last 6 orders for the user
    orders = ordersall[:6]
    ...
    if ordersall:
        ...
        order_today = ordersall.filter(
            date_created__range=[startdate_today, enddate_today])
        ...
        order_last_30day = ordersall.filter(
            date_created__range=[enddate_30, startdate_30])
    ...
    customers = sorted(
        customersall, key=lambda x: x.order_set.all().count(), reverse=True)[:6]
...
def customer(request, id):
    ...
    customer = get_object_or_404(Customer, id=id, user=user)
    orders = customer.order_set.all().order_by('-date_created')
    total_order = orders.count()

    startdate = date.today()
    enddate = startdate + timedelta(days=1)
    order_today = customer.order_set.all().filter(
...
def updatecustomer(request, id):
    ...
    customer = get_object_or_404(
        Customer, id=id, user=user)  # .objects.get(id=id)
